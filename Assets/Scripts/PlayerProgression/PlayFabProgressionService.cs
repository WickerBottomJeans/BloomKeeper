...
namespace DefaultNamespace
{
    public class PlayFabProgressionService
    {
        ...
        public Task<PlayerProgressionData> LoadProgression(PlayFabAuthSession authSession)
        {
            ...
            var completion = new TaskCompletionSource<PlayerProgressionData>(TaskCreationOptions.RunContinuationsAsynchronously);
            ...
            {
                HandleLoadProgressionResult(result, completion);
            }, error =>
            {
                completion.SetException(new InvalidOperationException($"PlayFab LoadProgression failed: {error.GenerateErrorReport()}"));
            });

            return completion.Task;
        }

        private static void HandleLoadProgressionResult(ExecuteFunctionResult result, TaskCompletionSource<PlayerProgressionData> completion)
        {
            try
            {
